...
import { escapeRegExp, getRazorpayConfig, getShopifyConfig, getWhatsappConfig, sanitizePhoneNumber } from 'utils/usefulfunction';
     ...
     try {
      ...
      //    //discount_code
         ...
         if (campaign.filters.is_payment_gateway_filter_enabled) {
           if (
             campaign.filters.discount_code_filter_any.length > 0 &&
             !campaign.filters.discount_code_filter_any.includes(orderById.paymentGatewayNames)
           ) {
             return;
           }
           ...
           if (
             campaign.filters.discount_code_filter_none.length > 0 &&
             campaign.filters.discount_code_filter_none.includes(orderById.paymentGatewayNames)
           ) {
             return;
           }
         }
      ...
      //  // Abandoned Checkout Logic
       ...
       if(campaign.related_order_fulfilled && orderById.displayFulfillmentStatus !== 'UNFULFILLED'){
        return
       }

       const linkOrderWithCampaign =await  this.databaseService.orderCampaign.create({
         data: {
           orderId: orderId
           ,
