...
export class GemniService {
  ...
  async generateEnumValues(userMessage: string): Promise<any> {
    // The prompt includes instructions for categorization and handling an order ID if present.
    const prompt = `
Assume you are a call center manager responding to user feedback on WhatsApp for a Shopify store. Your primary task is to handle user queries effectively. Given the following user message:
"${userMessage}"

Determine which category it falls into based on the options below:
...
If the user message contains an order ID  (for example,email,phoneno etc within a string like "{{12345}} or email or phone"), extract that ID and include it in your output. If no order ID is provided, set "orderId" to null.
...
`;
    ...
    const chatSession = this.model.startChat({
      ...
      history: [
        // Provide the prompt as a history message so the model sees the desired output format.
        { role: 'user', parts: [{ text: prompt }] },
        // Sample model response for guidance (example with an order ID):
        {
          role: 'model',
          parts: [
            {
    ...
    try {
      ...
      // Parse the response, trimming any extra whitespace.
